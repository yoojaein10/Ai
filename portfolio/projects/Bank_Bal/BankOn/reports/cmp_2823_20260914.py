"""2823 국민 — 담당자 완성본 읽기전용 대조(드라이런 --no-touch) + 전 칸 덤프(--dump). 저장 없음. 관리자 python."""
...
DOC, REQ = "01-2609-3-2823", dt.date(2026, 9, 9)
ts = time.strftime("%Y%m%d_%H%M%S")
log = open(ROOT / "reports" / f"cmp_2823_{ts}.log", "w", encoding="utf-8", errors="replace", buffering=1)
...
try:
    ...
    for tab in ("작성", "발송완료"):
        ...
        if navigate.find_row_by_doc(session, DOC, max_rows=30):
            found = tab; break
    if found is None:
        raise navigate.NavigationError("작성·발송완료 탭에 2823 없음")
    print(f"[runner] 탭={found}")
    ...
    print("")
    print("======== A) 드라이런 대조(--no-touch) ========")
    r = autofill_kb.main([DOC, "--no-touch"]); print(f"[runner] autofill_kb dry exit={r}")
    print("")
    print("======== B) 화면 전 칸 덤프(--dump) ========")
    r = autofill_kb.main([DOC, "--dump"]); print(f"[runner] dump exit={r}")
    w = driver.window(form.handle)
    for i in range(n):
        navigate.kb_select_object_row(form, i); w.set_focus(); time.sleep(0.4)
        w.capture_as_image().save(str(ROOT / "reports" / f"cmp_2823_{ts}_obj{i+1}.png"))
    rc = 0
