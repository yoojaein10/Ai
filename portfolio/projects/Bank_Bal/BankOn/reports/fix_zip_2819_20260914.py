...
log = open(ROOT / "reports" / f"fix_zip_2819_{ts}.log", "w", encoding="utf-8", errors="replace", buffering=1)
...
try:
    ...
    n = navigate.count_object_rows(f); print(f"행 수 {n}")
    if n != 6:
        raise RuntimeError(f"행 수 {n} ≠ 6 — 추가 행이 남았거나 폼이 바뀜, 저장 안 함")
    src = zip_of(f, 0); print(f"1행 우편번호={src!r}")
    if not src.isdigit() or len(src) != 5:
        raise RuntimeError("1행 우편번호가 5자리 숫자가 아님 — 중단")
    wrote = 0
    for i in range(1, n):
        navigate.select_object_row(f, i); time.sleep(0.4)
        res = F.fill(f, {"우편번호": src}, live=True, overwrite=False)
        for r in res:
            print(f"  행 {i+1}: {r.action} 현재={r.current!r} → {r.ours!r} wrote={r.wrote}")
            wrote += int(r.wrote)
    print(f"입력 {wrote}칸")
    if wrote:
        snaps = navigate.save_form(session, f, tag="zip2819"); print(f"저장 확인창: {snaps}")
    navigate.close_context_menu(); navigate.close_forms(session)

    # ③ 재확인
    print("\n== ③ 재확인 ==")
    f = open_doc()
    n = navigate.count_object_rows(f)
    print(f"행 수 {n} 우편번호: {[zip_of(f, i) for i in range(n)]}")
    w = driver.window(f.handle); w.set_focus(); time.sleep(0.3)
    w.capture_as_image().save(str(ROOT / "reports" / f"fix_zip_2819_{ts}_row6.png"))
    navigate.close_context_menu(); navigate.close_forms(session)
    rc = 0
