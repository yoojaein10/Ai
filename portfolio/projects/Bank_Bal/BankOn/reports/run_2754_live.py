# 2754 1건 LIVE — 큐 워커 경유(처리중/완료 이력이 남아 배포 PC 워커와 이중 실행 방지). 관리자 권한으로 띄움.
...
out = open(ROOT / "reports" / "worker_2754_live.out", "w", encoding="utf-8", errors="replace", buffering=1)
sys.stdout = out; sys.stderr = out
try:
    import run_queue_worker as W
    rc = W.main(["--live", "--once", "--seq", "12352", "--statuses", "진행"])
