...
# 지번 비교키 — 표기가 제각각인 지번(`52-9,` · `0052-0009` · `산 12-3` · `197-50외 1필지`)을
# 같은 것끼리 견주려고 앞의 0 과 꼬리를 떼어 `본번-부번` 으로 맞춘다. 다현장 문서에서 **물건이 어느 현장인지**
# 가리는 데 쓴다(2818: 옹정리 197-50 / 삼성동 52-9).
_JIBUN_KEY = re.compile(r"(?:산\s*)?(\d+)(?:\s*-\s*(\d+))?")


def jibun_key(text: str | None) -> str | None:
    """지번 비교키 — `0052-0009` → `52-9`, `197-50외 1필지` → `197-50`. 지번이 없으면 None.

    부번이 0(또는 없음)이면 본번만 낸다(`708-0` 과 `708` 을 같게 본다).
    """
    match = _JIBUN_KEY.match((text or "").strip())
    if not match:
        return None
    main_no = match.group(1).lstrip("0") or "0"
    sub_no = (match.group(2) or "").lstrip("0")
...
def dong_tail(address: str | None) -> str | None:
    """소재지의 **마지막 행정구역 낱말** — `경기도 김포시 통진읍 옹정리` → `옹정리`.

    의견서 개요 소재지는 시도·시군구가 빠지기도 해서(`옹정리 197-50`) 전체 문자열로는 못 견준다.
    꼬리(법정동 이름)만 맞춰 보면 앞이 짧아도 같은 현장인지 가릴 수 있다.
    """
    words = (dong_part(address) or "").split()
