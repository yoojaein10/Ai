...
class TestGuards:
    ...
    def test_평가단가는_토지에서만(self):
        ...
        assert building["평가단가"] == "0"          # 건물형 화면은 0 을 적는다(발송본 2672·2820, 2026-09-11)
...
class TestBuildingRules:
    ...
    def test_내용_잔존년수와_건물_평가단가는_0(self):
        # 소스가 없다. 화면은 전부 0 — 비워 두면 사람이 매번 0 을 쳤다(발송본 2672·2820 실측, 2026-09-11).
        values = nh.build(context(BUILD_2625))
        assert values["내용년수"] == "0" and values["잔존년수"] == "0" and values["평가단가"] == "0"

    # 실측 2672(2026-09-11): 대표주소·명세는 `이스턴스퀘어동 4-102호`(`제` 없음) 인데 공부스캔 건물 행은
    # `… 제이스턴스퀘어동 제1층 제4-102호` — 동·호·해당층수를 못 읽어 사람이 보충했다.
    GONGBU_2672 = (
        GongbuRow(seq_no="1", kind="토지", unique_no="2401-2026-018719",
                  address="서울특별시 강동구 천호동 580", area=Decimal("27393.1"), note="대"),
        GongbuRow(seq_no="2", kind="건물", unique_no="2401-2026-018719",
                  address="서울특별시 강동구 천호동 580 강동중흥에스클래스 제이스턴스퀘어동 제1층 제4-102호",
                  area=Decimal("25.7605"), note="전유부분 철근콘크리트구조"),
    )
    TABLES_2672 = {"section_build0": [
        {"JIBUN": "580"},
        {"NO": "가", "gujo": "철근콘크리트구조"},
        {"gujo": "제1층"},
        {"gujo": "제4-102호", "AREA1": "25.76", "AREA2": "25.76", "PRICE": "481,000,000"},
    ]}

    def test_동호층은_공부스캔_주소를_먼저_본다(self):
        values = nh.build(context(
            self.TABLES_2672, gongbu=self.GONGBU_2672,
            site_full="서울특별시 강동구 천호동 580 강동중흥에스클래스 이스턴스퀘어동 4-102호"))
        assert values["동"] == "이스턴스퀘어"
        assert values["호"] == "4-102" and values["해당층수"] == "1"

    def test_물건표시는_스캔주소의_지번_뒤(self):
        # 발송본 그대로: `580 강동중흥에스클래스 제이스턴스퀘어동 제1층 제4-102호`
        values = nh.build(context(
            self.TABLES_2672, gongbu=self.GONGBU_2672,
            site_full="서울특별시 강동구 천호동 580 강동중흥에스클래스 이스턴스퀘어동 4-102호"))
        assert values[nh.BOX_DISPLAY] == "580 강동중흥에스클래스 제이스턴스퀘어동 제1층 제4-102호"

    def test_물건표시는_스캔이_없으면_조립한다(self):
        values = nh.build(context(BUILD_2625, site_full="경기도 하남시 망월동 968-1 미사센트럴프라자 제4층 제408호"))
        assert values[nh.BOX_DISPLAY] == "968-1 미사센트럴프라자 제4층 제408호"
        # 건물명을 못 뽑으면 지번·층·호만으로 조립한다.
        bare = nh.build(context(BUILD_2625, site_full="경기도 하남시 망월동 968-1 미사센트럴프라자"))
        assert bare[nh.BOX_DISPLAY] == "968-1 제4층 제408호"
        with_dong = nh.build(context(BUILD_2625,
                                     site_full="서울특별시 성동구 도선동 69 성동삼성쉐르빌 제101동 제145호"))
        assert with_dong[nh.BOX_DISPLAY] == "968-1 성동삼성쉐르빌 제101동 제4층 제408호"

    def test_토지형_물건표시는_지번만(self):
        assert nh.build(context(LAND_2616))[nh.BOX_DISPLAY] == "766"
...
class TestHeaderFields:
    ...
    def test_소유자명은_괄호소유자_채무자_우선(self):
        # 2820: 등기 소유자 한국자산신탁 ↔ 감정평가표 괄호(채무자) 김찬양 — 화면은 괄호 기준(담당자 지적 2026-09-11).
        values = nh.build(context(LAND_2616, rounds=(
            RoundInfo(client="농협은행 의왕시지부장", owner="한국자산신탁주식회사", debtor="김찬양"),)))
        assert values["소유자명"] == "김찬양"
    ...
    def test_표준지(self):
        ...
        assert values[nh.BOX_STD_SITE] == "대부북동1472-5"   # 위치표기 키(2026-09-07)
