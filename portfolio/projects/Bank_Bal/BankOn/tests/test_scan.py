...
class TestIdentify:
    ...
    # 실측 01-2608-3-2739(한남동 657-97): 스캔 DB 에 **인천 용현동 573-7** 공부가 잘못 붙어 있었다.
    FOREIGN = [
        ("1", "토지", "1201-2022-037655", "인천광역시 미추홀구 용현동 573-7",
         Decimal("1637.6000"), "대"),
        ("2", "건물", "1201-2022-037655",
         "인천광역시 미추홀구 용현동 573-7 인하한양아이클래스 제11층 제1123호",
         Decimal("20.0700"), "전유부분 철근콘크리트구조"),
    ]

    def test_다른_필지_공부만_있으면_None(self):
        # 예전 폴백(첫 행)이 인천 등기번호를 한남동 물건에 넣었다(2026-09-11) → fail-closed.
        rows = scan.fetch_rows(FakeCursor(self.FOREIGN), "x")
        assert scan.registry_for(rows, kind="토지", jibun="657-97", area=Decimal("218")) is None
        # 지번을 모르면(안 주면) 종전대로 고른다.
        assert scan.registry_for(rows, kind="토지", area=Decimal("218")) == "12012022037655"
