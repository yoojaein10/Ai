...
try:
    ...
    # 물건 그리드 2·3행(건물 등)을 골라 오른쪽 탭이 어떻게 바뀌는지 덤프
    try:
        g0 = driver.by_class(form.handle, "TcxGridSite")[0]
        g0.click_input(); time.sleep(0.4); send_keys("^{HOME}"); time.sleep(0.4)
        for n in (2, 3):
            send_keys("{DOWN}"); time.sleep(1.0)
            snap_window(form.handle, f"write_row{n}")
            dump(form.handle, f"작성 폼 물건 {n}행 선택")
        send_keys("^{HOME}"); time.sleep(0.4)
    except Exception as e:
        print(f"[recon] 행 이동 덤프 실패 {e!r}")
    ...
    # 작성 폼 PDF등록 창(탭 구성 확인만, 파일 선택 없음)
    try:
        pdfw = navigate.open_pdf_window(session, form)
        print(f"[recon] 작성 PDF 창 {pdfw.class_name} '{pdfw.title}'")
        snap_window(pdfw.handle, "write_pdf")
        dump(pdfw.handle, "작성 PDF 창")
        navigate.close_pdf_windows(session, on_prompt=lambda s, **k: navigate.decline_save_prompt(s, wait=k.get("wait", 1.0)))
    except Exception as e:
        print(f"[recon] 작성 PDF 창 실패 {e!r}")
