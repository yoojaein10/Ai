"""★실서버 쓰기★ 기업 한 건 통합 러너(관리자 권한, 콘솔 숨김 필수). run_kb_full 과 같은 흐름 — 기업 차이:
    작성폼 PDF창은 '(원본)감정평가서' 탭만 씀(수수료·공부 탭 없음) · 현장조사서 PDF 는 '<번호> 현장조사.pdf' 또는 번호 없는 '현장조사서.pdf'
    (archive.find_survey_pdf) · 현장조사서 소계 4칸은 '자동계산' 버튼 4개 클릭 후 대조(autofill_survey) · DRY 는 --no-touch(행 추가·콤보도 안 건드림, --touch 로 해제).

    작성(A) 열기 → 입력(텍스트·콤보·물건 행) → 저 장 → PDF등록(전례 PDF 자동 탐색) → 폼 닫기
    → 포커스 행 확인(필요할 때만 재순회) → 현장조사서(E) 열기 → 입력 → 저 장 → 닫기
    → 현장조사서 PDF등록 → 검증(--verify): 두 폼 다시 열어 드라이런 대조 + PDF 창 재개봉 스크린샷.   발송(B/G)은 하지 않는다.

    Start-Process python -ArgumentList '"...\\tools\\run_ibk_full.py" <from> <to> <doc> [옵션]' -Verb RunAs -WindowStyle Hidden -Wait
    --dry        입력·저장·PDF 없이 계획만(안전)        --no-pdf    PDF등록 건너뜀
    --verify     저장 뒤 두 폼을 다시 열어 재대조(기본 생략)
    --pdf <경로> 전례 탐색 대신 이 파일               --overwrite 이미 있는 값도 덮어씀
    --survey-pdf <경로> 현장조사서 PDF(기본: 전례 폴더 자동 탐색, 없으면 시작 전 중단)
...
DRY_TOUCH = [] if "--touch" in OPTS else ["--no-touch"]   # DRY 기본은 화면을 전혀 안 건드림(발송완료 대조용)
...
try:
    ...
    if not SKIP_WRITE:
        ...
        # '사람 작성 → 제외'(재시도 없음)로 조용히 끝난다(오제외 실증 2026-09-15 2873·2836).
        if not form.class_name.startswith("TBNKKIB"):
            raise RuntimeError(f"기업 폼이 아닙니다: {form.class_name} (은행 판별 오류?)")
        human_guard.assert_not_written(form, "기업", summary, force=FORCE)   # 사람이 작성한 폼이면 Excluded(exit 3)
        r1 = autofill_ibk.main([DOC] + (DRY_TOUCH if DRY else ["--live"]) + OVERWRITE)
        print(f"[runner] autofill_ibk exit={r1} · 스냅샷 {snap(form.handle, 'write_filled')}")
    ...
    if not DRY and VERIFY:
        ...
        if not SKIP_WRITE:
            ...
            form = navigate.open_write_form(session, home=False)
            v1 = autofill_ibk.main([DOC, "--no-touch"])
