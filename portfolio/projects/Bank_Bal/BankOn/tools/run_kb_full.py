"""★실서버 쓰기★ 국민 한 건 통합 러너(관리자 권한, 콘솔 숨김 필수). run_shinhan_full 과 같은 흐름 + 현장조사서 PDF등록(국민만).

    작성(A) 열기 → 입력(텍스트·콤보·물건 행) → 저 장 → PDF등록(전례 PDF 자동 탐색) → 폼 닫기
    → 포커스 행 확인(필요할 때만 재순회) → 현장조사서(E) 열기 → 입력 → 저 장 → 닫기
    → 현장조사서 PDF등록(같은 PDF) → 검증: 두 폼 다시 열어 드라이런 대조 + PDF 창 재개봉 스크린샷.   발송(B/G)은 하지 않는다.

    Start-Process python -ArgumentList '"...\\tools\\run_kb_full.py" <from> <to> <doc> [옵션]' -Verb RunAs -WindowStyle Hidden -Wait
    --dry        입력·저장·PDF 없이 계획만(안전)        --no-pdf    PDF등록 건너뜀
    --verify     저장 뒤 두 폼을 다시 열어 재대조(기본 생략)
    --pdf <경로> 전례 탐색 대신 이 파일               --overwrite 이미 있는 값도 덮어씀
    --survey-pdf <경로> 현장조사서 PDF(기본: 같은 폴더 '<번호> 현장조사.pdf', 없으면 시작 전 중단)
...
NO_RELATED_PDF = "--no-related-pdf" in OPTS   # 수수료·공부 PDF 탭 첨부 생략(기본은 첨부, 파일 없으면 그 탭만 건너뜀)
RELATED_TABS = (("수수료", "(관련)수수료 등"), ("공부", "관련서류"))   # (파일 종류, PDF창 탭) — 사용자 지시 2026-08-28
...
try:
    ...
    related_pdfs: list[tuple[str, str, Path]] = []   # (종류, PDF창 탭, 파일) — 국민 작성폼 수수료·공부 탭 첨부(없는 것은 건너뜀)
    if not NO_PDF and not NO_RELATED_PDF and not SKIP_WRITE:
        for kind, tab in RELATED_TABS:
            path = locate(kind, None)
            if path:
                print(f"[runner] {kind} PDF → 탭 '{tab}': {path.name}")
                related_pdfs.append((kind, tab, path))
    ...
    if not SKIP_WRITE:
        ...
        # '사람 작성 → 제외'(재시도 없음)로 조용히 끝난다(오제외 실증 2026-09-15 2867).
        if not form.class_name.startswith("TBNKKBB"):
            raise RuntimeError(f"국민 폼이 아닙니다: {form.class_name} (은행 판별 오류?)")
        human_guard.assert_not_written(form, "국민", summary, force=FORCE)   # 사람이 작성한 폼이면 Excluded(exit 3)
        r1 = autofill_kb.main([DOC] + ([] if DRY else ["--live"]) + OVERWRITE)
        print(f"[runner] autofill_kb exit={r1} · 스냅샷 {snap(form.handle, 'write_filled')}")
        ...
        if not DRY:
            ...
            if pdf_path:
                ...
                navigate.ensure_pdf_uploaded(result)   # 업로드 미완이면 탭 전환·닫기 없이 실패(PdfUploadPending)
                for kind, tab, path in related_pdfs:   # 같은 PDF창에서 탭만 바꿔 이어서
                    if not any(w.handle == pdf_win.handle for w in navigate.pdf_windows(session)):
                        raise RuntimeError(f"PDF 창이 사라져 '{tab}' 첨부를 못 했습니다")
                    result = navigate.attach_pdf(session, form, str(path), tab=tab, pdf_win=pdf_win)
                    print(f"[runner] PDF등록 탭 '{tab}'({path.name}): 확인창={result['prompts']} "
                          f"소요={result['elapsed']}s 뷰어={'표시됨' if result.get('viewer') else '미표시'}")
                    print(f"[runner] 스냅샷 {snap(pdf_win.handle, 'pdf_' + kind)}")
                    summary[f"pdf_{kind}"] = f"{result['elapsed']}s prompts={result['prompts']}"
    ...
    if not SKIP_SURVEY:
        ...
        if not DRY:
            ...
            if survey_pdf_path:
                if not any(w.handle == survey.handle for w in navigate.survey_forms(session)):
                    raise RuntimeError("저장 후 현장조사서 폼이 사라졌습니다 — PDF등록 전 확인 필요")
                result = navigate.attach_pdf(session, survey, str(survey_pdf_path))
    ...
    if not DRY and VERIFY:
        ...
        if not SKIP_WRITE:
            ...
            form = navigate.open_write_form(session, home=False)
            v1 = autofill_kb.main([DOC, "--no-touch"])
