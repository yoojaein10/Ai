"""새마을금고 담보(TBNKMGB24DAMB) — 정품추출 → mgb 매핑 → "채울값 ↔ 화면" 대조 / 채우기.

우리판(verify_fill_wrb)의 새마을판. 화면에 물건 하나씩, 물건일련번호로 넘긴다.
인계본(우리_새마을_인계번들 2026-09-08) → 이 계통 이식 2026-09-10.
인계 검증: 자동순회 73건(통합형·일단지 member 계산 포함), 실입력 드라이런 대조 2762 ✅29/0.
...
KNOWN_KINDS = frozenset({"토지", "토지+건물", "집합건물", "건물"})   # 물건종류 4콤보 전부 부동산
...
def guard_reasons(ctx, values: dict, resolved) -> list[str]:
    """실입력 보류 사유 — 물건종류를 못 정하면(None) 보류(콤보 4종은 전부 부동산)."""
    kind = values.get("물건종류")
    return [] if kind in KNOWN_KINDS else [f"물건종류 미판정={kind!r}"]
...
def fill_mgb(cfg, doc: str, form, *, live: bool = False, overwrite: bool = False, seq=None):
    gam = process_gam(cfg.gamexport_exe, str(VK.fetch_gam_local(cfg, doc)),
                      str(Path("output") / doc))
    ctx = vf.build_context(cfg, doc, gam.tables, vf.sections_from_gam(gam))
    screen = vf.screen_values(form)
...
def main(argv=None) -> int:
    ...
    if not forms:
        ...
        print(f"새마을 담보폼({FORM_CLASS})이 안 열려 있습니다. (열린: {others or '없음'})", file=sys.stderr)
        return 1
