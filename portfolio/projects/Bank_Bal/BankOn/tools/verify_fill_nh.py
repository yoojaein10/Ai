"""농협 담보(TBNKNHB24DAMB) — 정품추출 → nh 매핑 → "채울값 ↔ 화면" 대조 / 채우기.

국민·기업판(verify_fill_kb / verify_fill_ibk)의 농협판. 읽기 전용(채우기는 --live 옵션이
있는 순회 도구에서만).

  화면에 농협 담보폼 열어둔 상태에서:
    python tools/verify_fill_nh.py 01-2608-3-2625
"""
...
# 호번호 해석은 `parse.address` 한 곳에만 둔다(사본이 갈리지 않게).
...
# 화면이 원본 그대로 0을 채워 넣는 칸 — `0618` 과 `618` 은 같은 지번이다.
...
def resolve_seq(ctx, screen, seq=None) -> str | None:
    ...
    농협은 같은 문서에 **감정평가표가 여러 장**일 수 있고, 그때 물건마다 금액이 같을 수도
    있다(실측 2645: 두 물건 다 338,000,000). 그래서 금액만으로는 못 가른다 —
    **면적 → 호 → 금액 → 화면 일련번호** 순으로 좁힌다.
    """
    if seq:
        return str(seq)
    rows = _property_rows(ctx)
    # 화면 `물건기호` 는 명세행 NO(가/나 · 1/2)를 그대로 쓴다 — 가장 확실한 키다.
    # 금액이 없는 물건도 화면에 물건①로 뜰 수 있어(실측 2052 '가' 는 PRICE 가 비었다)
    # 금액 앵커 목록으로는 못 고른다.
    # 기호가 숫자가 아니고(가/나/다) **그 기호를 가진 행이 하나뿐일 때만** 그대로 넘긴다 —
    # `detail.for_sequence` 가 NO 로 직접 찾는다. 감정평가표가 여러 장이면 같은 기호가
    # 표마다 있어(실측 2645: 두 표 모두 '가') 첫 행을 집는다 → 그땐 면적으로 가려야 한다.
    # 숫자 기호는 화면 슬롯 번호와 구분이 안 되므로 아래 정합으로 넘긴다.
    mark = norm(screen.get("물건기호", ""))
    if mark and not mark.isdigit() and sum(1 for r in rows if r.seq_no == mark) == 1:
        return mark
    ...
    area = norm(screen.get("사정면적", "") or screen.get("공부면적(전용면적)", "")
                or screen.get("공부면적", ""))
    if area:
    ...
    if amount.isdigit():
        ...
        if len(matched) == 1:                    # 금액이 겹치면 확정키가 못 된다
...
def seq_alignment(ctx, screen, seq=None) -> dict:
    """국민·기업과 같은 계약의 순번 정합 정보(순회 채우기가 공용으로 쓴다)."""
    ...
    if index < 0 and resolved:
        # 한글 물건기호('가'/'나')로 직접 짚은 경우 — 그 기호를 가진 행이 하나뿐이라 확정키다
        # (2625 실측: 화면 물건기호 '가' → 종전엔 int() 못 해 '불명확'으로 실입력이 막혔다, 2026-09-07).
        marked = [i for i, r in enumerate(anchors) if r.seq_no == resolved]
        if len(marked) == 1:
            return {"total": total, "screen_seq": screen_seq, "resolved": resolved,
                    "by": "물건기호", "safe": True}
    if 0 <= index < total:
        row = anchors[index]
        our_area = f"{row.area_assessed or row.area_public:.2f}" \
            if (row.area_assessed or row.area_public) is not None else ""
        screen_area = norm(screen.get("사정면적", "") or screen.get("공부면적(전용면적)", ""))
...
def build_fill_nh(cfg, doc: str, form, seq=None):
    """정품추출 → nh 매핑 → "채울값 ↔ 화면" 대조행. 그리드 순회용."""
    ...
    mine = nh.build(ctx, resolve_seq(ctx, screen, seq))
...
def fill_nh(cfg, doc: str, form, *, live: bool = False, overwrite: bool = False,
            select: bool = False, seq=None):
    """농협 담보 폼에 매핑값을 채운다(기본 드라이런). 순회 채우기용."""
...
def main(argv=None) -> int:
    ...
    if not forms:
        ...
        print(f"농협 담보폼({FORM_CLASS})이 안 열려 있습니다. (열린: {others or '없음'})",
              file=sys.stderr)
        return 1
    rows = build_fill_nh(cfg, args.doc_id, forms[0], args.seq)
    print(f"===== {args.doc_id} — 농협 auto-fill 채울값 ↔ 화면 =====")
