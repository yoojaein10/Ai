...
def _open(cfg, doc_id: str) -> driver.WindowRef:
    # 자동 로그인 값은 **여기서만** 쓴다 — 검증 도구는 사람이 띄워 둔 화면을 읽기만 하므로
    # `.env` 에 없어도 된다(`config.AppConfig` 에서 선택 항목). 필요할 때 분명히 알린다.
    if not (cfg.loader_cmd and cfg.bankon_user and cfg.bankon_password):
        raise ConfigError(
            "BANK24 자동 로그인 설정(BANKON_LOADER_CMD·BANKON_USER·BANKON_PASSWORD)이 "
            "`.env` 에 없습니다. 직접 로그인해 두고 검증 도구를 쓰거나, 세 값을 채우세요.")
