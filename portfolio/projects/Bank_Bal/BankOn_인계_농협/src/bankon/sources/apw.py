...
출처(실물 검증 완료):
  ...
  **소재지**       apw_masterex.ADDR — 시도·시군구·동까지 완성된 문자열.
                  농협 14건 중 13건이 화면 `소재지` 칸과 **글자 그대로** 같았고, 기업
                  2683·2526 도 화면값과 일치했다. (안 맞은 1건은 다물건 문서라 화면이
                  두 번째 물건을 보고 있던 것 — 주소 자체는 정확.)
  물건 표시        apw_masterex.Address — ADDR + 지번 + 건물명 + 층/호가 이어진 자유 문자열.
                  건물명 소스로 쓴다(농협 8건 중 5건, 나머지는 의견서 개요로 폴백).
