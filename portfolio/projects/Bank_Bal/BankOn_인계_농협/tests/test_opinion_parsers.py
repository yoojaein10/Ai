...
class TestDateParsing:
    ...
    def test_gamexport_로캘_날짜도_ISO로(self):
        """gamexport 는 **추출한 PC 의 로캘**로 날짜를 찍는다.

        원본 PC 는 ISO 였지만 로캘이 다른 PC 에서 재추출하면 `8/12/2026` 이 된다
        (실측 2526: 캐시=2026-08-12 / 재추출=8/12/2026). 정규화하지 않으면 화면
        날짜 칸에 이 문자열이 그대로 입력된다.
        """
        assert outline.to_iso_date("8/12/2026") == "2026-08-12"
        assert outline.to_iso_date("8/25/2026") == "2026-08-25"

    def test_앞자리가_12를_넘으면_일월로_읽는다(self):
        # 유럽식 로캘 PC 대비 — 25는 월일 수 없다.
        assert outline.to_iso_date("25/8/2026") == "2026-08-25"

    def test_ISO_는_그대로다(self):
        # 정규화는 멱등이어야 한다(이미 검증된 원본 PC 추출물이 안 바뀌도록).
        assert outline.to_iso_date("2026-08-12") == "2026-08-12"
