...
from bankon.parse import (  # noqa: E402
    address, characteristics, cost, detail, kb, mullist, outline, sections, standard_land,
)
...
def build_context(cfg, doc_id: str, tables: dict, section_body) -> DocumentContext:
    ...
    # 화면 소재지는 시군구까지 필요한데 개요 소재지엔 없는 건이 많다 — 본문에서 찾는다.
    # 이미 아는 짧은 주소(개요·명세)를 힌트로 줘야 '…계획관리' 같은 오검출을 피한다.
    ...
    return DocumentContext(
        ...
        cost_layers=layers,
        price_point_date=info.get("PricePointDate"),
        survey_date=info.get("SearchStartDate") or None,
