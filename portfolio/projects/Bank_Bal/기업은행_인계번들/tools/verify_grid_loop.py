...
def main(argv=None) -> int:
    ...
    p.add_argument("--bank", choices=("shg", "kb", "ibk"), default="shg",
                   help="shg=신한 kb=국민 ibk=기업")
    p.add_argument("--fill", action="store_true",
                   help="검증 대신 '채우기 계획'(드라이런) — 국민·기업")
    p.add_argument("--fill-live", action="store_true",
                   help="실제 입력(빈칸만) — 국민·기업. 반드시 감독 하에!")
    ...
    def process(form, fid) -> tuple[str, str | None]:
        ...
        if args.fill or args.fill_live:            # 채우기 모드(국민·기업)
            ...
            if args.fill_live and not align["safe"]:
                ...
                print(f"{head}  {doc}  ✍입력 {done} · 미발견 {miss}{seqinfo}")
            else:
                print(f"{head}  {doc}  📝채울계획 {plan} · 미발견 {miss}{seqinfo}(드라이런)")
            return "clean", doc
        try:
            if args.bank == "kb":                        # 국민: 다물건이면 화면 금액으로 물건 정합
                rows = VF_KB.build_fill_kb(cfg, doc, form)
                sb, mb = _bun_from_rows(rows)
            elif args.bank == "ibk":                     # 기업
                rows = VF_IBK.build_fill_ibk(cfg, doc, form)
                sb, mb = _bun_from_rows(rows)
            else:
