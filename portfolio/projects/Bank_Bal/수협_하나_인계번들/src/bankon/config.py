...
class AppConfig:
    ...
    # BANK24 **자동 로그인**에만 쓰는 값들 — 사람이 직접 로그인하면 필요 없다.
    # 검증·대조 도구는 화면을 읽기만 하므로 이 셋이 없어도 돌아야 한다(없으면 None).
    loader_cmd: str | None = None     # BANK24 실행 명령(KadcLoader.exe Bank24 -e)
    bankon_user: str | None = None    # 뱅크온라인 로그인 아이디
    bankon_password: str | None = None  # 뱅크온라인 로그인 비밀번호
...
def load_config(env_path: str | None = None) -> AppConfig:
    ...
    return AppConfig(
        ...
        gamexport_exe=_require("GAMEXPORT_EXE"),
        work_dir=_require("WORK_DIR"),
        output_dir=_require("OUTPUT_DIR"),
        business_number=_require("BUSINESS_NUMBER"),
        loader_cmd=_optional("BANKON_LOADER_CMD"),
        bankon_user=_optional("BANKON_USER"),
        bankon_password=_optional("BANKON_PASSWORD"),
        scan_sql=scan_sql,
        dw_sql=dw_sql,
