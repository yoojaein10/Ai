...
BOX_DISPLAY = f"물건표시{_BOX}[6]"             # 지번+건물명+층/호 — 사람이 다듬어 쓴다
...
def building_name(context: DocumentContext, item) -> str | None:
    ...
    """
    from .kookmin import _building_name_from  # 지번 뒤 이름을 떼는 규칙은 국민과 같다
...
def _floor_ho_dong(item, context) -> tuple[str | None, str | None, str | None]:
    ...
    **지번 뒤부터만** 본다. 층·호·동 표기는 늘 지번 다음에 오는데, 앞쪽 주소에는
    ...
    texts = [t for t in (tail(item.location if item else None),
    ...
    for text in (tail(context.site_full), tail(item.location if item else None)):
...
def build(context: DocumentContext, seq_no: str | None = None) -> dict[str, str | None]:
    ...
    values: dict[str, str | None] = {
        ...
        "평가단가": _money(item.unit_price if (item and is_land) else None),
    ...
    if is_building:                          # 건물형 화면에만 있는 칸
        values.update({
            ...
            "내용년수": None,                 # 실측 화면 전부 0 — 소스가 없어 사람이 넣는다
            "잔존년수": None,
