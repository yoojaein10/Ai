...
MAIN_CLASS = "TfrmMain"
# ⚠️ `TfrmMain` 은 Delphi 기본 폼 이름이라 **다른 프로그램**(위치도·ApWorks 등)도 같은 클래스를 쓴다.
#    메인 창은 반드시 제목으로도 걸러야 한다(실측 2026-09-08: BANK24 가 꺼진 상태에서 '위치도_New' 를 잡았다).
MAIN_TITLE = "BANK24"
...
def window_matches(name: str, title: str, class_name: str | None = None,
                   title_contains: str | None = None) -> bool:
    """창 클래스(접두 일치)·제목(부분 일치) 조건 — `find_windows` 의 필터(순수 함수, 테스트용)."""
    if class_name and not (name or "").startswith(class_name):
        return False
    if title_contains and title_contains not in (title or ""):
        return False
    return True


def find_windows(class_name: str | None = None, pid: int | None = None,
                 title: str | None = None) -> tuple[WindowRef, ...]:
    """조건에 맞는 최상위 창 목록. `title` 은 제목 부분 일치(메인 창은 `MAIN_TITLE` 로 걸러라)."""
    try:
    ...
    for element in elements:
        ...
        if not window_matches(name, (element.name or ""), class_name, title):
            continue
...
def wait_for_window(
    ...
    *,
    ...
    while time.monotonic() < deadline:
        windows = find_windows(class_name, pid, title=title)
...
def launch(loader_cmd: str, *, timeout: float = DEFAULT_TIMEOUT) -> WindowRef:
    ...
    """
    existing = find_windows(MAIN_CLASS, title=MAIN_TITLE) or find_windows(LOGIN_CLASS)
    if existing:
        return existing[0]
