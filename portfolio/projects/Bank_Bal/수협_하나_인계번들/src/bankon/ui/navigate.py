...
def open_app(loader_cmd: str, user: str, password: str, *, timeout: float = 90.0) -> Session:
    ...
    메인 창은 클래스(`TfrmMain`)만으로 잡으면 안 된다 — Delphi 기본 폼 이름이라 다른 프로그램
    (위치도·ApWorks 등)의 창이 먼저 잡힌다(실측 2026-09-08). 제목 `BANK24` 로 같이 거른다.
    """
    already = driver.find_windows(driver.MAIN_CLASS, title=driver.MAIN_TITLE)
    if already:
    ...
    main = driver.wait_for_window(driver.MAIN_CLASS, timeout=timeout, title=driver.MAIN_TITLE)
    time.sleep(2.0)                      # 툴바·그리드가 채워질 때까지 잠깐(로그인 직후 빈 껍데기 실측)
    return Session(main)
...
def close_forms(session: Session, *, timeout: float = 20.0) -> None:
    ...
    `닫 기` 는 **메시지로** 누른다(`click()` = BM_CLICK). 좌표 클릭(`click_input`)은
    마우스를 그 지점으로 옮겨 누르는 거라, 폼이 앞에 없으면 그 자리에 있는 **다른 창을
    누른다** — 실제로 폼이 안 닫힌 채 다음 문서를 열려다 그리드 포커스를 잃었다.
    폼이 남아 있으면 그 값을 다음 문서의 화면값으로 잘못 읽게 되므로 조용히 넘기지 않는다.

    누르기 전에 **폼을 포그라운드로 올린다** — 뒤에 깔린 폼에 BM_CLICK 을 보내면 반응하지
    않는 경우가 있다(실측: 사람이 직접 열어 둔 폼이 20초를 기다려도 안 닫혔다).
    """
    for form in open_forms(session):
        ...
        try:
            ...
            pass                                 # 포커스는 거들 뿐 — 실패해도 눌러 본다
        try:
            button.click()                       # BM_CLICK — 마우스를 뺏지 않는다
        except Exception:
            driver.click(button)                 # 최후수단: 좌표 클릭
    ...
    if remaining:
        # 안내창이 떠 있으면 제목을 같이 알려 준다 — 무엇을 눌러야 할지는 **사람이** 정한다.
        popups = [d.title for cls in ("#32770", "TMessageForm")
                  for d in driver.find_windows(cls)]
        hint = f" 화면에 뜬 창: {popups}" if popups else " 화면에 뜬 안내창이 있는지 확인하세요."
        raise NavigationError(
            f"담보 폼이 닫히지 않았습니다({', '.join(f.class_name for f in remaining)}) —{hint}")
...
def clear_search(session: Session) -> None:
    """검색칸(감정서조회·의뢰번호조회)을 비운다.

    **조회는 이 칸을 필터로 쓴다** — 값이 남아 있으면 기간을 어떻게 잡아도 그 건만
    나오거나 0건이 된다(실측: 앞 문서번호가 남은 채 기간을 바꿔 조회해 계속 0건이었다).
    """
    for label in (SEARCH_LABEL, "의뢰번호조회"):
        edit = driver.find_by_label(session.main.handle, label, "TcxTextEdit")
        if edit is not None and driver.read(driver.editable(edit)):
            driver.set_text(edit, "", verify=False)


def requery(session: Session, *, settle: float = 4.0) -> None:
    """화면에 잡혀 있는 조건 그대로 **`조 회`** 를 다시 눌러 목록을 복원한다.

    `찾 기`(감정서번호·의뢰번호)로 한 건만 남긴 뒤 전건 순회를 하려면 목록을 되돌려야
    한다. 조회 조건(업무구분·기간)은 **건드리지 않는다** — 화면에 사람이 잡아 둔 그대로
    다시 조회할 뿐이다. 검색칸은 비운다(안 비우면 그 건만 다시 나온다).
    """
    clear_search(session)
...
def set_date_range(session: Session, start: str, end: str | None = None) -> None:
    """조회 기간을 바꾸고 다시 조회한다(YYYY-MM-DD).

    `찾 기` 는 **조회된 목록 안에서만** 찾는다 — 기간 밖 문서는 번호를 넣어도 안 나온다.
    그래서 번호로 문서를 열려면 기간이 그 문서를 덮고 있어야 한다.
    """
    clear_search(session)          # 검색칸이 남아 있으면 기간을 바꿔도 0건이 된다
    for label, value in (("조회시작일", start), ("조회종료일", end)):
        if value is None:
            continue
        edit = driver.find_by_label(session.main.handle, label, "TcxDateEdit")
        if edit is None:
            raise NavigationError(f"'{label}' 칸을 찾지 못했습니다.")
...
# 문서번호 앞부분이 접수 연월이다 — `01-2608-3-2625` → 2026년 8월.
_DOC_MONTH = re.compile(r"^\d{2}-(\d{2})(\d{2})-")


def range_for(doc_id: str, *, months: int = 1) -> tuple[str, str] | None:
    """문서번호의 접수 연월 앞뒤 `months` 달을 덮는 조회 기간.

    ⚠️ **기간을 몇 년으로 넓히면 BANK24 가 0건을 돌려준다**(실측 2024-01-01~2027-12-31 → 0건).
    그래서 넓히는 게 아니라 **그 문서가 있는 달로 옮긴다.**
    """
    match = _DOC_MONTH.match(doc_id)
    if not match:
        return None
    ...
    start = first - timedelta(days=31 * months)
    end = first + timedelta(days=31 * (months + 1))
...
def open_document(
    session: Session, doc_id: str, *, timeout: float = 60.0, widen: bool = True
) -> driver.WindowRef:
    """문서번호 하나를 작성 폼까지 열어 준다.

    한 번 실패하면 **조회 기간을 그 문서의 접수 달로 옮겨** 다시 시도한다. `찾 기` 는 조회된
    목록 안에서만 찾기 때문에, 화면에 잡힌 기간이 그 문서를 안 덮으면 번호가 맞아도 0건이
    되고 컨텍스트 메뉴의 '작 성(열람)' 이 먹지 않는다(실측: 기간이 3월 한 달로 좁혀져 있어
    8월 문서를 못 열었다).
    """
    ...
    try:
        ...
        close_context_menu()
        span = range_for(doc_id) if widen else None
        if span is None:
            raise
    set_date_range(session, *span)
    find_document(session, doc_id)
    try:
        return open_write_form(session, timeout=timeout)
    except Exception:
