...
# 라벨이 없는 칸 — **위치**로 짚는다(`ui.form.between_labels`). 화면값 163건
# (`recon/kb_screen.json`)으로 확인했다.
#
#  · `소재지`(181,570)는 자기 라벨이 없다. 밴드 `기계기구`~`건물명` 에서 **가장 왼쪽**이
#    163/163 성립한다(순번은 폼 변형마다 달라 못 쓴다).
#  · `법정동코드`(307,548)도 라벨이 없는데 **쓸 수 있는 규칙이 없다**. 국민 폼은 토지 건에
#    `토지탭`이 겹쳐 뜨면서 좌패널 칸들을 복제하고(번지구분·본번지·부번지가 각 2칸)
#    점검항목 라벨을 빌려 쓴다. 그래서 밴드 경계가 문서마다 흔들려 순번이 160/163 에서만
#    맞는다 — 3건이 어긋나면 **이웃 칸에 코드를 타이핑**하게 되므로 아예 내지 않는다.
BOX_SITE = "소재지@기계기구~건물명[왼쪽]"
...
def _address_only(text: str | None) -> str | None:
    ...
    # **지번 앞에서 자른다**(은행 공용 규칙). 예전엔 마지막 `…동/리/가` 까지 탐욕 매칭했는데,
    # 건물명에 든 `씨동`·`상가동`까지 먹어 소재지에 건물명이 딸려 들어갔다(실측 28건:
    # `경기도 의왕시 포일동 653 인덕원아이티밸리 씨동` 전체를 소재지로 냈다).
    return address.dong_part(head)


# 국민 화면은 용도지역을 **약칭**으로 쓴다 — `제1종일반주거지역` → `1종일반주거`.
# 앞의 `제` 와 뒤의 `지역`/`구역` 을 뗀다(실측 163건 화면값: 계획관리·2종일반주거·
# 자연녹지·개발제한·일반상업·준주거 …). 기업은 반대로 **정식 명칭 그대로**다.
#
# 다만 등급도 수식도 없는 단독 명칭은 통째로 쓴다(실측 `농림지역`). 이 칸은 콤보라
# 규칙이 빗나가도 목록에 없는 값은 못 고르고 그대로 비므로(ESC 복원) 위험하지 않다.
_ZONE_WHOLE = frozenset({"농림지역"})
_ZONE_TRIM = re.compile(r"(지역|구역)$")


def zone_short(text: str | None) -> str | None:
    """화면 용도지역 약칭. 실측 163건 중 규칙이 서는 79건을 맞춘다(나머지는 사람 재량)."""
    name = common.zone_name(text)
    if not name:
        return None
    name = unicodedata.normalize("NFKC", name)
    if name in _ZONE_WHOLE:
        return name
    return _ZONE_TRIM.sub("", re.sub(r"^제", "", name)) or None
...
def _building_name(context: DocumentContext) -> str | None:
    ...
    """
    return _building_name_from(context.kb_summary.address)
...
def build(context: DocumentContext, seq_no: str | None = None) -> dict[str, str | None]:
    ...
    elif item and item.is_land and item.group_head:
        # 묶음머리(일단지)는 AREA2/PRICE 가 **묶음 합계**다. 화면은 대개 물건 하나만 보므로
        # 머리 필지 단독값(AREA1, AREA1×단가)을 쓴다.
        #
        # 화면값 163건으로 묶음머리 20건을 재 봤다 — **화면=AREA1 15건 / AREA2 4건 / 불명 1건**.
        # 예전엔 소수 쪽(AREA2)을 썼다. 가르는 소스 신호를 찾아봤지만(등기 수·묶음 필지 수·
        # 소분 유무·면적 비율) 두 무리를 나누는 게 없었다 — 공부 스캔이 있으면 등기 합병
        # 여부로 갈릴 것 같은데 이 20건엔 스캔이 하나도 없다. 그래서 **다수 쪽**을 쓴다.
        # 남는 4건(0033·0663·0916·1344)은 사람이 고친다.
        if item.area_public is not None:
            item_assessed = item.area_public
            if item.unit_price is not None:
                item_amount = item.area_public * item.unit_price
    ...
    return {
        ...
        # 법정동코드 칸은 라벨이 없고 **위치로도 못 짚는다** — 아래 BOX_SITE 주석 참조.
        # 사람이 채운다(주소검색이 같이 채워 준다).
        "번지구분": jibun.jibun_kind if jibun else None,
        "본번지": main_no,
        "부번지": sub_no,
        BOX_SITE: _address_only(_address(context)),
        ...
        "용도지역": zone_short(item.zone if item else outline.zone),
