"""기업은행 담보 폼 자동입력 — 매핑값을 화면에 채운다(되읽기 검증, 순번 정합).

verify_fill_ibk 가 "채울값 ↔ 화면" 검증이라면, 이건 그 값을 **실제로 입력**한다.
기본은 **드라이런**(계획만). 실제 입력은 `--live` 로만.

⚠️ 실제 입력은 반드시 **연습/더미 감정서**로 먼저 시험하세요. 텍스트·숫자·날짜칸만 자동
   입력하고, 선택형(콤보·라디오)은 '수동'으로 남깁니다. 다물건은 화면 감정평가액으로 물건
   순번을 정합해 채우며, 어느 물건인지 불명확하면 실입력을 보류합니다.

    (화면에 기업은행 담보폼 열어둔 상태)
    python tools/autofill_ibk.py 01-2608-3-2683            # 드라이런(계획만)
    python tools/autofill_ibk.py 01-2608-3-2683 --live     # 실제 입력(연습건에서만!)
"""
...
def _seq_safe(ctx, screen, resolved) -> bool:
    """다물건이면 화면 감정평가액이 우리 물건과 맞는지(=금액매칭) 확인. 단일은 항상 안전."""
    anchors = [r for r in ctx.details if (r.is_land or r.is_building) and r.amount is not None]
    if len(anchors) < 2:
        return True
    idx = (int(resolved) - 1) if (resolved and str(resolved).isdigit()) else 0
    our_amt = str(int(anchors[idx].amount)) if 0 <= idx < len(anchors) else ""
    scr_amt = VI.norm(screen.get("감정평가액", ""))
    if scr_amt and scr_amt == our_amt:
        return True                              # 금액 일치 = 물건 확정
    scr_seq = VI.norm(screen.get("일련번호", ""))
    return scr_seq.isdigit() and 1 <= int(scr_seq) <= len(anchors)
...
def main(argv=None) -> int:
    ...
    if not forms:
        print("기업은행 담보 화면(TBNKKIB24DAMB)이 안 열려 있습니다.", file=sys.stderr)
        return 1
    form_ref = forms[0]
    ...
    resolved = VI.resolve_seq(ctx, screen, args.seq)
    values = ibk.build(ctx, resolved)

    mode = "🔴 실제입력(LIVE)" if args.live else "🟢 드라이런(계획만, 화면 안 건드림)"
    print(f"===== {args.doc_id} 기업 자동입력 — {mode} =====")
    anchors = [r for r in ctx.details if (r.is_land or r.is_building) and r.amount is not None]
    safe = _seq_safe(ctx, screen, resolved)
    if len(anchors) >= 2:                          # 다물건 순번 정합 표시
        flag = "✅정합" if safe else "⚠불명확"
        print(f"  📍다물건 {len(anchors)}건 · 화면 일련번호={VI.norm(screen.get('일련번호','')) or '?'} "
              f"→ 채울 물건 순번={resolved or '1'} {flag}")
    do_live = args.live and safe
    results = form.fill(form_ref, values, live=do_live, overwrite=args.overwrite,
                        select=args.select)
    if args.live and not safe:
