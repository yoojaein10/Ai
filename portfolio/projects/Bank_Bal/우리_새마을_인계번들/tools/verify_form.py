...
from bankon.model import DocumentContext, Fee, Parties  # noqa: E402
from bankon.parse import (  # noqa: E402
    address, characteristics, cost, detail, kb, mullist, outline, round as round_parse,
    sections, standard_land,
)
...
BANK_FORMS = {"TBNKSHG24DAMB": "신한", "TBNKKBB24DAMB": "국민",
              "TBNKKIB24DAMB": "기업", "TBNKNHB24DAMB": "농협"}
...
def sections_from_gam(gam):
    """gamexport 결과의 의견서 → 섹션 조회 함수."""
    opinion = max(gam.opinions, key=lambda o: len(o.document.text), default=None)
    secs = sections.split(opinion.document.paragraphs) if opinion else ()
...
def build_context(cfg, doc_id: str, tables: dict, section_body) -> DocumentContext:
    ...
    parsed_outline = outline.parse(section_body("대상물건 개요"))
    ...
    # 본문 탐색은 이미 아는 짧은 주소(개요·명세)를 힌트로 줘야 '…계획관리' 오검출을 피한다.
    site_address = site.address or address.full_address(
    ...
    return DocumentContext(
        ...
        cost_layers=layers,
        # gamexport 는 날짜를 **추출한 PC 의 로캘**로 찍는다(ISO 인 PC 도 있고 `8/12/2026`
        # 인 PC 도 있다) — 화면에 그대로 넣으면 안 되므로 여기서 ISO 로 못 박는다.
        ...
        details=details,
        gam_category=(info.get("category") or None),
    )
...
def screen_values(form: driver.WindowRef) -> dict[str, str]:
    ...
    for field in collect(walk(_connect(driver.BACKEND, title_re=None, pid=None,
        ...
        if "RadioGroup" in field["class_name"]:
            continue
        # 값이 **자기 라벨과 같은** 칸은 값이 아니라 **열 머리**다(국민 토지탭의
        # `물건종류`·`평가방법`). 비교에 넣으면 전건이 가짜 불일치로 올라온다.
        if str(field["value"]).strip() == str(field["label"] or "").strip():
            continue
...
def verify_one(cfg, args, doc_id: str, form, bank) -> tuple[int, int, int]:
    ...
    try:
        ...
        if bank == "신한":
            combos = load_combos(Path(__file__).resolve().parent.parent
                                 / "recon" / "combo_shinhan.md")
