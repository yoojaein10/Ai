...
Begin
	...
	(
		...
		From APW_IW_MANDAYWORK A
			...
			Left join Apw_MasterEx d On a.Docid = d.Docid	
		Where
			Convert(varchar(10), A.workdate, 23) like  LEFT(@F_Date,7) + '%' 
			and A.GUBUN = 1  
			And B.Gubun =2  
			And C.OFFICE_ID='10'
	...
	(
		...
		From APW_IW_MANDAYWORK A
			...
			Left join Apw_MasterEx d On a.Docid = d.Docid	
			Where
			Convert(varchar(7), workdate, 23) = LEFT(@F_Date,7) 
			and A.GUBUN = 1  
			And B.Gubun =6  
			And C.OFFICE_ID='10'
			--And A.Manager = @SManager
	...
	GROUP BY a.Price, a.Manager
		...
		Into #TmpIJub
				From APW_MASTER A
				Left join APW_YJI_StandardPrice B ON B.Stand_Year =@StandYear
				left outer join (	SELECT MasterID, 	max( case when cia.iType = 1 then cia.Names else NULL end) as 'Manager'
				FROM APW_Charge_IDX cia 
													group by MasterID
											) cia on cia.MasterID = A.MasterID 
		 Where
		 ...
		 Select Count(*) ISim_Cnt
			...
			(
				...
				From APW_simsa_form a
					...
					Left join Apw_MasterEx d On a.Docid = d.Docid	
				Where 1=1
					And Convert(varchar(10), A.simsadate, 23) like  LEFT(@F_Date,7) + '%' 
					And B.Gubun =11  
					And d.Manager not like '공(%'
					And d.Office = '10'
			) a
	...
	--	 (
			...
			Where 1=1
				And Convert(varchar(10), Reg_DateTime, 23) like  LEFT(@F_Date,7) + '%' 
		  ...
		  SELECT Count(A.DocID) ISIJO_Cnt,
			...
			Into #TmpISIJO
		        ...
		        Left join APW_Masterex C ON A.docid =  C.DocID
				 Where
				 Convert(varchar(10), simsadate, 23)like  LEFT(@F_Date,7) + '%'  
				 And B.Gubun =12  
	...
	End
      ...
      ---협회심사비 
		...
		(
			...
			Where 1=1
			   And isnull(Docid, '') = ''
				...
				And Convert(varchar(10),JunpyoDate, 23) like Left(@F_Date, 7) + '%'
		...
		Where 1=1
			 And isnull(a.Docid, '') <> ''
			And Convert(varchar(10), a.JunpyoDate, 23) like Left(@F_Date, 7) + '%'
	...
	(
		...
		From Hug_info a
			Left join Apw_YJI_StandardPrice b On b.Stand_Year = @StandYear
		Where 1=1
			And Convert(varchar(20), a.ReceiptDate, 23) like Left(@F_Date, 7) + '%'
			And b.Gubun = '19'
	) a
	Group by a.Price
	...
	(
		...
		From Hug_info a
			Left join Apw_YJI_StandardPrice b On b.Stand_Year = @StandYear
		Where 1=1
			And Convert(varchar(20), a.Complete_Date, 23) like Left(@F_Date, 7) + '%'
			And b.Gubun = '20'
	) a
	Group by a.Price
