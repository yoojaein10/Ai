...
def fetch_current_docids(db_config: dict, cust_docids, office: str = "10") -> dict:
    """APW_Master에서 현재 감정서번호(DocID)를 의뢰번호별로 조회한다 (읽기전용, ? 바인딩).

    수기 BankOnline 전송 직전에 호출한다 — 저장 이후 감정서번호가 바뀌었어도
    항상 조회 시점의 DB 값을 GAM_NO로 쓰기 위함. 미존재 의뢰번호는 결과에서 빠진다.

    반환: {"ok": bool, "docids": {의뢰번호: DocID}, "error": str}
    """
    normalized = list(dict.fromkeys(
        d for d in (normalize_cust_docid(v) for v in cust_docids) if d
    ))
    if not normalized:
        return {"ok": True, "docids": {}, "error": ""}
    ...
    try:
        ...
        for start in range(0, len(normalized), BATCH):
            ...
            sql = (
                f"SELECT CustDocID, DocID FROM dbo.APW_Master "
                f"WHERE Office = ? AND CustDocID IN ({placeholders})"
            )
            cur.execute(sql, [str(office)] + batch)
            for row in cur.fetchall():
                key = normalize_cust_docid(row[0])
        ...
        try:
            ...
            pass
        return {"ok": True, "docids": docids, "error": ""}
    except Exception as e:
        return {"ok": False, "docids": {}, "error": type(e).__name__}
    finally:
        if conn is not None:
            try:
                conn.close()
...
_HANA_BANK_ALIASES = ("KEB하나은행", "하나은행")


def _bank_name_variants(bank_name: str) -> list:
    """APW_Customer 조회용 은행명 후보(우선순위순).
    하나은행은 'KEB하나은행'/'하나은행' 표기가 혼재 등록되어 양쪽 모두 조회한다."""
    if bank_name in _HANA_BANK_ALIASES:
        return [bank_name] + [b for b in _HANA_BANK_ALIASES if b != bank_name]
...
def lookup_cust_code(conn, bank_name: str, branch_name: str) -> dict:
    ...
    bank_variants = _bank_name_variants(bank_name)

    # 정확매칭 후보(우선순위순): 은행명 표기 변형 × 영업점
    exact_names = []
    for b in (bank_variants or [""]):
        fn = f"{b} {branch_name}".strip() if branch_name else b
        if fn and fn not in exact_names:
    ...
    def _rank(rows):
        def key(r):
            ...
            s = 0
            if active == 'Y':                              s -= 100
            # 영업점명 어절 경계 매칭 우선: '계동지점'이 '상계동지점'의
            # 부분 문자열로 오탐되는 것을 방지 (공백 뒤 또는 이름 시작)
            if branch_name and any(
                f" {bv}" in cname or cname.startswith(bv)
                for bv in branch_variants
            ):
                s -= 50
            if bank_name and cname.startswith(bank_name):
                s -= 20
            elif bank_variants and any(cname.startswith(b) for b in bank_variants):
                s -= 10
    ...
    def _try(active_only):
        ...
        # 1차: 정확 매칭 (은행명 표기 변형 우선순위순)
        for fn in exact_names:
            rows = _q(["CustName=?"], [fn], active_only)
            if rows:
                return _rank(rows), f"exact_active{suffix}" if active_only else f"exact{suffix}"

        # 2차: LIKE 부분 매칭 (은행명 변형 OR + branch_name 변형 포함)
        for bv in branch_variants:
            if bank_variants and bv:
                bank_conds = " OR ".join(["CustName LIKE ?"] * len(bank_variants))
                rows = _q(
                    [f"({bank_conds})", "CustName LIKE ?"],
                    [f"{b}%" for b in bank_variants] + [f"%{bv}%"],
                    active_only,
                )
                if rows:
        ...
        if bank_variants:
            bank_conds = " OR ".join(["CustName LIKE ?"] * len(bank_variants))
            rows = _q([f"({bank_conds})"], [f"{b}%" for b in bank_variants], active_only)
            if rows:
...
def _resolve_customer_lookup_bank_name(item: dict) -> str:
    ...
    if bank_name == "수협은행" and regional:
        ...
        if compact:
            return compact
    if bank_name == "농협중앙회":
        branch = (item.get("영업점", "") or "").strip()
        if branch.endswith("농협"):
            expanded = re.sub(r"농협$", "농업협동조합", branch)
            if expanded:
                return expanded
    return bank_name
