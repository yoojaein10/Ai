...
# ── 우리은행 담당자(최상단 행) 좌표 기반 추출 ──────────────────────────────────
#
# 우리은행 의뢰서는 '담당자명 / 전화번호' 라벨 행이 페이지 상단에 두 번 나온다.
# 범용 텍스트 파서는 라인 순서에 의존해 아래쪽(두 번째) 행을 고르는 경우가 있어,
# 여기서는 page.get_text("words")의 좌표만으로 '가장 위쪽(첫 번째) 담당자 행'을
# 결정론적으로 선택한다. 빨간 박스·주석·편집기 표시에는 의존하지 않는다.
#
# y 허용오차: 같은 시각적 행의 단어라도 글리프 상승부(ascent)·베이스라인 차이로
# get_text가 돌려주는 y0 값이 소수점 단위로 흔들릴 수 있다. 이 양식의 행 간격은
# 약 22pt이므로, 그보다 훨씬 작은 3pt 밴드로 묶으면 같은 행만 결합하고 다음 행으로
# 번지지 않는다. (단어/객체 저장 순서가 아니라 좌표로만 판정한다.)
_WOORI_Y_TOL = 3.0

def _select_woori_top_contact(words):
    """words(get_text("words") 형식)에서 최상단 담당자 행의 (담당자, 담당자 연락처).

    순수 함수: 파일 I/O 없이 좌표 리스트만 받는다. 단어 반환 순서에 의존하지 않으며
    좌표로만 판정한다. 이름·전화번호가 모두 기존 검증을 통과할 때만 값을 채우고,
    조건을 못 채우면 해당 값은 None으로 둔다.
    각 word 튜플: (x0, y0, x1, y1, text, block_no, line_no, word_no).
    """
    # '담당자명' 라벨 단어들 — y0가 가장 작은(=페이지 최상단) 것을 첫 행으로 정의.
    # 반환 순서가 아니라 좌표로만 선택하므로 결정론적이다.
    mgr_labels = [w for w in words if w[4].strip() == '담당자명']
    if not mgr_labels:
        return None, None
    top = min(mgr_labels, key=lambda w: (w[1], w[0]))
    y = top[1]

    # 동일 y-밴드 단어만 결합 (x 오름차순).
    band = sorted(
        (w for w in words if abs(w[1] - y) <= _WOORI_Y_TOL),
        key=lambda w: w[0],
    )
    phone_labels = [w for w in band if w[4].strip() == '전화번호']

    # 담당자: '담당자명' 라벨 오른쪽 ~ '전화번호' 라벨 왼쪽 사이의 첫 유효 이름.
    name_x_min = top[2]  # 라벨 오른쪽 끝
    name_x_max = min((w[0] for w in phone_labels), default=float('inf'))
    name = None
    for w in band:
        if w[0] < name_x_min or w[0] >= name_x_max:
            continue
        t = w[4].strip()
        if _is_employee_name_candidate(t):
            name = t
            break

    # 담당자 연락처: '전화번호' 라벨 오른쪽의 숫자 조각들을 좌→우로 결합.
    # 번호가 여러 단어로 쪼개져 저장될 수 있어 조각을 이어붙인 뒤,
    # 공백 제거·연속 공백 축약 후 기존 형식 규칙(_is_phone)으로 검증한다.
    phone = None
    if phone_labels:
        pl = min(phone_labels, key=lambda w: w[0])
        frags = []
        for w in band:
            if w[0] <= pl[2]:
                continue
            ...
            if re.fullmatch(r'[\d\-() ]+', t):
                ...
                break
        cand = re.sub(r'\s+', ' ', ''.join(frags)).strip()
        if _is_phone(cand):
            phone = cand
...
def _extract_woori_top_contact(pdf_path: str):
    """우리은행 의뢰서 PDF에서 최상단 담당자 행의 (담당자, 담당자 연락처)를 반환.

    page.get_text("words") 좌표만 사용한다. 실패 시 (None, None)을 반환하고
    예외·raw 텍스트·PII를 외부(로그/stdout/예외)로 흘리지 않는다.
    """
    try:
        ...
        try:
            ...
            doc.close()
        return _select_woori_top_contact(words)
    except Exception:
        return None, None
...
def parse_bank24_pdf(pdf_path: str) -> dict:
    ...
    try:
        ...
        # ── 우리은행 전용: 최상단 담당자 행 좌표 기반 선택 ────────────────────
        # 이름·전화번호가 모두 유효할 때만 신규 결과를 적용하고, 그 외에는
        # 위에서 구한 범용 결과를 그대로 사용한다(불완전/모호 시 fallback).
        if bank_nm == "우리은행":
            w_name, w_phone = _extract_woori_top_contact(pdf_path)
