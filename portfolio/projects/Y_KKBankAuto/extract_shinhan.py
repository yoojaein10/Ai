...
OUTPUT_DIR = Path(r"C:\Y_KKBankAuto\output")
...
_BANKONLINE_CUSTKEY = {
    ...
    "하나은행":         "HNB",
    "농협은행":         "NHB",
    "농협중앙회":       "NHB",  # 농협은행과 동일 CUSTKEY로 전송
    "우리은행":         "WRB",
    "수협은행":         "SSB",
    "새마을금고":       "MGB",
    "주택도시보증공사": "HUG",
...
# ══════════════════════════════════════════════════════════════════════════════
# BankOnline API 보안 로깅 유틸 (fail-closed)
#   - 허용 필드/안전코드만 기록. 자격증명·토큰·PII·요청/응답 본문은 남기지 않는다.
#   - 불확실하면 전송하지 않고 고정 안전코드만 남긴다.
# ══════════════════════════════════════════════════════════════════════════════
...
# error_code / SKIP reason 공용 allowlist (이 집합 밖의 값은 UNEXPECTED_ERROR로 대체)
_API_SAFE_CODES = frozenset({
    "DISABLED", "ENDPOINT_MISSING", "ENDPOINT_INVALID", "ENDPOINT_NOT_ALLOWED",
    "REDIRECT_REJECTED", "DB_FAILED", "NO_OUTPUT", "NO_NEWDOCID",
    "ITEM_MATCH_FAILED", "ROLLBACK_TEST", "TOKEN_CREDENTIALS_UNAVAILABLE",
    "TOKEN_REQUEST_FAILED", "CALLER_NOT_CONFIGURED", "PAYLOAD_BUILD_FAILED",
    "HTTP_TIMEOUT", "HTTP_CONNECTION_FAILED", "HTTP_STATUS_ERROR",
    "RESPONSE_TOO_LARGE", "INVALID_RESPONSE", "SUCCESS_FIELD_MISSING",
    "API_REJECTED", "DUPLICATE_ATTEMPT_BLOCKED", "UNEXPECTED_ERROR",
})

# 승인된 HTTPS 호스트 (정확 일치 비교, 문자열 suffix 검사 아님)
_API_ENDPOINT_HOSTS = frozenset({"webrest.kapanet.or.kr"})
_API_TOKEN_HOSTS    = frozenset({"authtoken.kapanet.or.kr"})


def _api_safe_code(code) -> str:
    """allowlist에 없는 값은 UNEXPECTED_ERROR로 강제 치환."""
    c = str(code or "")
    return c if c in _API_SAFE_CODES else "UNEXPECTED_ERROR"


class _ApiError(Exception):
    """allowlist 코드와 (선택) 정수 http_status만 전달하는 내부 예외.
    예외 원문 메시지를 로그로 흘리지 않기 위한 안전 컨테이너."""
    def __init__(self, code, http_status=None):
        self.code = _api_safe_code(code)
        self.http_status = http_status if isinstance(http_status, int) else None
        super().__init__(self.code)


def _api_scrub(text) -> str:
    """로그 문자열 정규화: C0/C1 제어문자·CR·LF·NUL 제거, 한 줄, 최대 길이 제한."""
    s = str(text)
    s = re.sub(r"[\x00-\x1f\x7f-\x9f]+", " ", s)   # C0/C1 + CR/LF/NUL/TAB
    s = " ".join(s.split())
    return s[:_API_LOG_MAXLEN]
...
def _api_emit(line: str) -> None:
    """[API] 로그 1줄 기록. 기록 실패는 무해화하되 API 판단은 바꾸지 않는다."""
    try:
        log(_api_scrub(line))
    except Exception:
        pass
...
class _ApiRefFactory:
    """실행 1회 salt로 원문 식별자를 비식별 ref(12자)로 변환.
    salt/원문/대응표는 어디에도 저장하지 않는다. 실행 종료 시 clear()."""
    __slots__ = ("_salt",)

    def __init__(self):
        self._salt = _api_secrets.token_bytes(16)

    def ref(self, raw) -> str:
        s = str(raw or "").strip()
        if not s or not self._salt:
            return "none"
        digest = _api_hashlib.sha256(self._salt + s.encode("utf-8")).hexdigest()
        return digest[:12]
...
def _api_check_endpoint(url, allowed_hosts):
    """endpoint 안전성 평가. 반환: (configured, https, allowed, code).
    code=='' 이면 통과. userinfo/빈 host/비-HTTPS/비승인 host를 거부한다.
    hostname 비교는 URL 파싱 결과의 정확 일치만 사용한다."""
    import urllib.parse
    raw = str(url or "").strip()
    if not raw:
        return (False, False, False, "ENDPOINT_MISSING")
    try:
        parsed = urllib.parse.urlsplit(raw)
    except Exception:
        return (True, False, False, "ENDPOINT_INVALID")
    scheme = (parsed.scheme or "").lower()
    netloc = parsed.netloc or ""
    # userinfo(사용자정보) 포함 URL 거부
    if parsed.username or parsed.password or "@" in netloc:
        return (True, scheme == "https", False, "ENDPOINT_INVALID")
    if scheme != "https":
        return (True, False, False, "ENDPOINT_INVALID")
    try:
        host = (parsed.hostname or "").lower()
    except Exception:
        return (True, True, False, "ENDPOINT_INVALID")
    if not host:
        return (True, True, False, "ENDPOINT_INVALID")
    if host not in allowed_hosts:
        return (True, True, False, "ENDPOINT_NOT_ALLOWED")
    return (True, True, True, "")


def _api_build_opener():
    """리다이렉트를 차단하는 opener. 각 리다이렉트 hop은 REDIRECT_REJECTED로 처리."""
    import urllib.request

    class _NoRedirect(urllib.request.HTTPRedirectHandler):
        def redirect_request(self, req, fp, code, msg, headers, newurl):
            raise _ApiError("REDIRECT_REJECTED", code if isinstance(code, int) else None)

    return urllib.request.build_opener(_NoRedirect)


def _api_http_status(response):
    try:
...
def _bankonline_enabled_flag(config: dict) -> bool:
    """[bankonline] enabled 플래그만 평가 (endpoint 설정 여부와 무관)."""
    rest = _bankonline_rest_config(config)
    return str(rest.get("enabled", "false") or "false").strip().lower() in ("1", "true", "yes", "on")


def _bankonline_rest_enabled(config: dict) -> bool:
    rest = _bankonline_rest_config(config)
    return _bankonline_enabled_flag(config) and bool(str(rest.get("endpoint", "") or "").strip())
...
def _call_bankonline_rest(payload: dict, config: dict) -> dict:
    """REST_BANK_JUBSU_UPD 호출. 성공 시 {"success": True, "http_status": int}.
    실패는 allowlist 코드를 담은 _ApiError로만 전달한다.
    payload/응답 본문/자격정보는 반환하거나 기록하지 않는다."""
    import json
    import socket
    import urllib.error
    import urllib.parse
    ...
    if not authorization:
        raise _ApiError("CALLER_NOT_CONFIGURED")

    # HTTPS + 승인 host allowlist (allow_http 설정과 무관하게 HTTPS만 허용)
    _, _, _, ep_code = _api_check_endpoint(endpoint, _API_ENDPOINT_HOSTS)
    if ep_code:
        raise _ApiError(ep_code)

    method = str(rest.get("method", "POST") or "POST").strip().upper()
    if method not in ("POST", "PUT", "PATCH"):
        raise _ApiError("PAYLOAD_BUILD_FAILED")
    ...
    opener = _api_build_opener()
    try:
        with opener.open(request, timeout=timeout) as response:
            # bounded-read: 최대 1MiB+1바이트까지만 읽는다 (전체 선(先)적재 금지)
            raw = response.read(1024 * 1024 + 1)
            if len(raw) > 1024 * 1024:
                raise _ApiError("RESPONSE_TOO_LARGE")
            status = _api_http_status(response)
    except _ApiError:
        raise
    except urllib.error.HTTPError as he:
        raise _ApiError("HTTP_STATUS_ERROR", getattr(he, "code", None))
    except (socket.timeout, TimeoutError):
        raise _ApiError("HTTP_TIMEOUT")
    except urllib.error.URLError:
        # DNS·연결·TLS 오류 일괄
        raise _ApiError("HTTP_CONNECTION_FAILED")
    except Exception:
        raise _ApiError("UNEXPECTED_ERROR")

    if status is None or not (200 <= status < 300):
        raise _ApiError("HTTP_STATUS_ERROR", status)

    try:
        decoded = json.loads(raw.decode("utf-8-sig")) if raw else {}
    except Exception:
        raise _ApiError("INVALID_RESPONSE", status)
    ...
    for part in success_field.split("."):
        if not isinstance(value, dict) or part not in value:
            raise _ApiError("SUCCESS_FIELD_MISSING", status)
        value = value[part]
...
def _fetch_bankonline_token(kapa_id: str, kapa_pw: str, config: dict) -> str:
    ...
    if not kapa_id or not kapa_pw:
        raise _ApiError("TOKEN_CREDENTIALS_UNAVAILABLE")
    ...
    _, _, _, ep_code = _api_check_endpoint(endpoint, _API_TOKEN_HOSTS)
    if ep_code:
        raise _ApiError(ep_code)
    ...
    try:
        with opener.open(request, timeout=30) as response:
            ...
            if len(raw) > 4096:
                raise _ApiError("TOKEN_REQUEST_FAILED")
    except _ApiError:
        raise
    except (socket.timeout, TimeoutError):
        raise _ApiError("TOKEN_REQUEST_FAILED")
...
def _call_bankonline_api(payload: dict, config: dict) -> tuple:
    """BankOnline send 1건 실행.
    반환: (ok: bool, safe_code: str, http_status: int|None)."""
    import socket
    import urllib.error

    if not payload:
        return (False, "PAYLOAD_BUILD_FAILED", None)
    ...
    if not callable(caller):
        return (False, "CALLER_NOT_CONFIGURED", None)
    try:
        response = caller(payload)
    except _ApiError as ae:
        return (False, ae.code, ae.http_status)
    except (socket.timeout, TimeoutError):
        return (False, "HTTP_TIMEOUT", None)
    except urllib.error.HTTPError as he:
        code = getattr(he, "code", None)
        return (False, "HTTP_STATUS_ERROR", code if isinstance(code, int) else None)
    except urllib.error.URLError:
        return (False, "HTTP_CONNECTION_FAILED", None)
    except Exception:
    ...
    if isinstance(response, dict) and response.get("success") is True:
        hs = response.get("http_status")
        return (True, "", hs if isinstance(hs, int) else None)
    return (False, "API_REJECTED", None)


def _apply_bankonline_updates(db_result: dict, items: list, config: dict) -> dict:
    """BankOnline API 스테이지. 순서대로 실행조건을 검사하고, 각 단계의 사유를
    [API] 안전 로그로 남긴다. 이 함수는 절대 예외를 전파하지 않으며 SUMMARY를
    정확히 1회 출력한다(정상·사전검사 실패·예외 종료 모두)."""
    from db_writer import normalize_cust_docid

    refs = _ApiRefFactory()
    counters = {"target": 0, "attempted": 0, "success": 0, "failed": 0, "skipped": 0}
    result = {"enabled": True, "tried": 0, "success": 0, "fail": 0, "blocked": 0, "errors": []}
    summary_state = {"done": False}
    token = None
    api_config = config

    def _summary():
        if summary_state["done"]:
            return
        summary_state["done"] = True
        _api_emit(
            f"[API][SUMMARY] target={counters['target']} attempted={counters['attempted']} "
            f"success={counters['success']} failed={counters['failed']} skipped={counters['skipped']}"
        )

    try:
        has_mock = callable(config.get("_bankonline_api_caller"))
        enabled = has_mock or _bankonline_enabled_flag(config)

        # 엔드포인트 평가. mock caller가 주입된 경우 실제 endpoint 검증은 생략한다.
        if has_mock:
            ep_configured = ep_https = ep_allowed = True
            ep_code = ""
            tok_code = ""
        else:
            rest = _bankonline_rest_config(config)
            ep_configured, ep_https, ep_allowed, ep_code = _api_check_endpoint(
                rest.get("endpoint", ""), _API_ENDPOINT_HOSTS)
            _, _, _, tok_code = _api_check_endpoint(
                rest.get("token_endpoint", "https://authtoken.kapanet.or.kr/AuthServer"),
                _API_TOKEN_HOSTS)

        _api_emit(
            f"[API][CONFIG] enabled={_api_bool(enabled)} "
            f"endpoint_configured={_api_bool(ep_configured)} "
            f"endpoint_https={_api_bool(ep_https)} endpoint_allowed={_api_bool(ep_allowed)}"
        )

        # ── 조건 1: enabled ──
        if not enabled:
            result["enabled"] = False
            _api_emit("[API][SKIP] ref=none reason=DISABLED")
            return result

        # ── 조건 2~4: endpoint / token_endpoint (HTTPS + 승인 host) ──
        if not has_mock:
            if ep_code:
                _api_emit(f"[API][SKIP] ref=none reason={_api_safe_code(ep_code)}")
                return result
            if tok_code:
                _api_emit(f"[API][SKIP] ref=none reason={_api_safe_code(tok_code)}")
                return result

        # ── 조건 5~6: DB 저장 성공 / output 존재 (PRECHECK) ──
        outputs = db_result.get("outputs") or []
        had_db_error = (
            bool(db_result.get("errors"))
            and not outputs
            and int(db_result.get("success", 0) or 0) == 0
        )
        db_success = not had_db_error
        _api_emit(
            f"[API][PRECHECK] db_success={_api_bool(db_success)} "
            f"output_count={_api_int_or_none(len(outputs))}"
        )
        if not db_success:
            _api_emit("[API][SKIP] ref=none reason=DB_FAILED")
            return result
        if not outputs:
            _api_emit("[API][SKIP] ref=none reason=NO_OUTPUT")
            return result

        # ── 조건 10~11: 토큰 자격정보 조회 + 발급 (REST 경로에서만) ──
        setup_error = ""
        if not has_mock:
            _api_emit("[API][TOKEN] request_start")
            rest = _bankonline_rest_config(config)
            try:
                ...
                try:
                    ...
                    pass
                _api_emit("[API][TOKEN] result=success http_status=none error_code=none")
                api_config = dict(config)
                api_config["_bankonline_authorization"] = token
            except _ApiError as ae:
                setup_error = ae.code
                _api_emit(
                    f"[API][TOKEN] result=failure http_status={_api_int_or_none(ae.http_status)} "
                    f"error_code={_api_safe_code(ae.code)}"
                )
            except Exception:
                setup_error = "TOKEN_REQUEST_FAILED"
                _api_emit("[API][TOKEN] result=failure http_status=none error_code=TOKEN_REQUEST_FAILED")

        # 건별 매칭용 item_map
        item_map = {}
        for it in items:
            key = normalize_cust_docid(it.get("상세창 의뢰번호") or it.get("의뢰번호", ""))
            if key:
                item_map[key] = it

        rollback = bool(db_result.get("rollback_test", False))
        attempted_refs = set()
        ...
        for output in outputs:
            ...
            # HUG: item_map 키는 그리드 의뢰번호 → 그리드_의뢰번호로 매칭
            norm_grid = normalize_cust_docid(
                output.get("그리드_의뢰번호") or output.get("의뢰번호", "")
            )
            new_doc_id = str(output.get("NewDocID", "") or "").strip()
            ref = refs.ref(norm_out or norm_grid)
            matched = norm_grid in item_map

            _api_emit(f"[API][ITEM] ref={ref} matched={_api_bool(matched)}")
            _api_emit(f"[API][NEWDOC] ref={ref} present={_api_bool(bool(new_doc_id))}")

            def _skip(code):
                sc = _api_safe_code(code)
                counters["skipped"] += 1
                result["blocked"] += 1
                output["BankOnline_ErrorType"] = sc
                _api_emit(f"[API][SKIP] ref={ref} reason={sc}")

            # ── 조건 9: rollback 테스트 아님 ──
            if rollback:
                _skip("ROLLBACK_TEST"); continue
            # ── 조건 7: NewDocID 존재 ──
            if not new_doc_id:
                _skip("NO_NEWDOCID"); continue
            # ── 조건 8: item 매칭 성공 ──
            item = item_map.get(norm_grid)
            if item is None:
                _skip("ITEM_MATCH_FAILED")
                result["errors"].append({"의뢰번호": norm_out, "error": "ITEM_MATCH_FAILED"})
                continue
            # ── 조건 10~11: 토큰 setup 실패 ──
            if setup_error:
                _skip(setup_error); continue
            # ── 조건 12: caller 구성 완료 ──
            if not _has_bankonline_caller(api_config):
                _skip("CALLER_NOT_CONFIGURED"); continue
            # ── 조건 13: payload 생성 + APPCODE 정확히 300617 ──
            payload = _build_bankonline_payload(item, new_doc_id, api_config)
            if not payload or payload.get("APPCODE") != _APPCODE:
                _skip("PAYLOAD_BUILD_FAILED"); continue
            # ── 조건 14: 동일 실행 내 중복 전송 차단 ──
            if ref in attempted_refs:
                _skip("DUPLICATE_ATTEMPT_BLOCKED"); continue
            attempted_refs.add(ref)

            # ── send (건별 최대 1회, 자동 재시도 없음) ──
            _api_emit(f"[API][SEND] ref={ref} request_start appcode={_APPCODE}")
            counters["attempted"] += 1
            result["tried"] += 1
            ok, code, http_status = _call_bankonline_api(payload, api_config)
            _api_emit(
                f"[API][SEND] ref={ref} result={'success' if ok else 'failure'} "
                f"http_status={_api_int_or_none(http_status)} "
                f"error_code={'none' if ok else _api_safe_code(code)}"
            )
            if ok:
                counters["success"] += 1
                result["success"] += 1
                output["BankOnline_In"]        = "Y"
                output["BankOnline_ErrorType"] = ""
            else:
                counters["failed"] += 1
                result["fail"] += 1
                ...
                output["BankOnline_In"]        = "N"
                output["BankOnline_ErrorType"] = sc
                result["errors"].append({"의뢰번호": norm_out, "error": sc})
            _api_emit(f"[API][RESULT] ref={ref} BankOnline_In={output['BankOnline_In']}")

    except Exception:
        # API 경로 예외는 원문을 남기지 않고 안전코드로만 흡수한다.
        _api_emit("[API][SKIP] ref=none reason=UNEXPECTED_ERROR")
    finally:
        _summary()
        # 토큰·salt 참조를 가능한 이른 시점에 제거 (완전 소거를 보장하지는 않음)
        try:
            token = None
            if isinstance(api_config, dict):
                api_config.pop("_bankonline_authorization", None)
        except Exception:
            pass
        try:
            refs.clear()
...
def run_extraction(config: dict, callbacks: dict = None):
    ...
    try:
        ...
        # API 스테이지는 항상 실행한다. 미사용(비활성)도 [API] 로그로 사유를 남기고
        # SUMMARY를 정확히 1회 기록해 '실행되지 않음'과 '실패'를 구분한다.
        ...
        on_summary({"counts": counts, "file": str(fname) if fname else "", "items": items, "db": db_result})
        set_step("저장", "done")
